...
<Tip>
  Make sure your GoHighLevel account has admin access to the location we’ll be
  optimizing. If you need help provisioning seats or setting up permissions,
  your Aidan strategist can walk through it on a call.
</Tip>

## Overview

The GoHighLevel integration enables Aidan to interact with your CRM, manage appointments, send messages across multiple channels, and trigger custom workflows—all automatically through your AI agents.

---

## Integration Setup

We usually walk through this process with you on a shared screen. If you need to complete it asynchronously, follow the steps below and send confirmation once finished.

### Step 1: Get Your Aidan API Key

First, you'll need to grab your API key from Aidan.
...
Locate and copy the Aidan API Key. You'll need this in a moment.
...
Still in Aidan, go to **Integrations** in the sidebar.
...
A GoHighLevel permission screen will appear, listing all the scopes Aidan needs to function properly:
...
**3.3 Choose Your Location**

Select which GoHighLevel location you want to connect to Aidan.

<Frame>
  <img src="/images/ghl-step10b-choose-location.png" alt="Choose location" />
</Frame>
...
Once your GoHighLevel integration is active, Aidan unlocks powerful capabilities:
...
Aidan provides powerful custom actions that you can use directly within your GoHighLevel workflows. These actions enable you to automate conversations, make phone calls, and trigger AI modules seamlessly.
...
<AccordionGroup>
  <Accordion title="Run AI Module" icon="microchip">
    Execute Aidan tools and modules directly within your GoHighLevel workflows to prefetch data, run calculations, or trigger complex automations.

    **What it does:**

    This action allows you to run any Aidan Tool or tool as part of your GHL workflow. You can use this to:
    ...
    - **Integrate with external APIs** through Aidan's tool ecosystem
...
<AccordionGroup>
  <Accordion title="❌ Invalid API Key Error">
    ...
    **Solution:** Double-check that you copied the Aidan **Company-level** API key, not the GoHighLevel account's API key. These are two different credentials—you need the one from Aidan's Company settings.
  </Accordion>
...
<Accordion title="🔒 Permissions Error">
  **Problem:** Authorization fails or you see a permissions error. **Solution:**
  Make sure you're logged into the correct GHL location before authorizing. The
  permissions need to be granted by an admin user with sufficient access rights
  in GoHighLevel.
</Accordion>
...
<Accordion title="🏢 Multiple Companies Setup">
  ...
  **Solution:** You'll need to repeat the integration steps for each client
  separately. Every GoHighLevel instance requires its own connection to Aidan.
  Navigate to each company context in Aidan and complete the authorization flow.
</Accordion>

...
title: "Voice Agents"
icon: "phone"
description: "Manage and optimise the voice agents already live for your business"
---

## Overview

Voice agents are the powerhouse of Aidan—perfect for outbound dialing, lead qualification, appointment booking, and more. We’ve already launched the core experiences for you; use this guide to review settings, refine scripts, and keep every call aligned with your brand.

<Note>
  **Need prompt inspiration?** Check out our [Prompting Guide](/prompting-guide)
  for best practices on refining agent instructions.
</Note>
...
<Frame>
  <img src="/images/voice-agent-prompt.png" alt="Core Prompt Settings" />
</Frame>

### Prompt

Edit the prompt whenever you need to adjust tone, qualification rules, or escalation logic. This text is the brain of your voice agent—it defines personality, objectives, and how to handle different scenarios.
...
<Tip>
  Use the **Generate** button to brainstorm alternative copy, then edit the
  output so it matches your brand voice. Always place a test call after major
  edits to verify flow and pronunciation.
</Tip>
...
```
"Aidan, Jacob speaking—how can I help?"
```
...
Review the tools already connected to your agent. You can adjust configurations, enable/disable specific actions, or add new ones as your workflow evolves.
  ...
  <Accordion title="Transfer Tool" icon="phone-arrow-right">
    ...
    **Transfer Types:**
    ...
    <AccordionGroup>
      ...
      <Accordion title="Warm Transfer with Agent" icon="headset">
        **Concierge transfer** – Agent rings the destination first, gives context, and confirms whether to connect the caller.

        - ✅ Prevents cold handoffs—recipient opts in before the caller joins
        - ✅ Agent delivers a short recap and captures any instructions
        - ✅ If the recipient declines, the agent ends the transfer gracefully and updates the caller
      </Accordion>
    </AccordionGroup>
    ...
    **Example Setup:**
    ...
    - Transfer Type: Warm Transfer with Message (e.g. "Customer needs help with [specific issue]")

    Destination 3: VIP Escalations
    - Description: "When the caller is a high-value client or requests a manager"
    - Message: "One moment while I check if our lead consultant is available to speak with you."
...
- **Chinese (China)** – Simplified Chinese (China)
- **Chinese (Simplified)** – Simplified Chinese
- **Chinese (Traditional)** – Traditional Chinese
- **Chinese (Taiwan)** – Traditional Chinese (Taiwan)
...
<Tip>
  **Browse All Available Voices:** Visit
  [app.callaidan.ai/voice-library](https://app.callaidan.ai/voice-library) to
  listen to samples and preview the complete voice collection before making your
  selection.
</Tip>

Aidan offers access to ElevenLabs' massive voice library with over 1,000 professionally trained voices covering:
...
- **Custom Upload (MP3)** – Bring your own ambience by uploading an MP3 file; perfect for branded office loops or on-site recordings.
- **Mute** – No background sound (clean audio only)

**Upload a Custom Background:**

1. Click **Upload custom MP3** and choose your file.
2. Wait for the upload to finish—your track will appear alongside the presets.
3. Save the agent to make the custom sound live on calls.

<Note>
  We recommend short, subtle loops so the audio blends into the call.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Website Scrape" icon="globe">
    Paste any URL and Aidan will scrape the entire site into clean, AI-readable Markdown.
...
```
Aidan Mandurah Fremantle Joondalup CrossFit Pilates HIIT
```
...
**Tell Aidan how to format the call summary posted back to your CRM.**
...
If left blank, Aidan generates a standard summary automatically.
...
<AccordionGroup>
  ...
  Before rolling changes to production, open **Voice Settings → Preview** to sample the voice with your current prompt. Place a quick test call to yourself to evaluate clarity, tone, and professionalism, then lock in the winner.
</Accordion>
